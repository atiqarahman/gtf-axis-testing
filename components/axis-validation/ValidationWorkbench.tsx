  ...
  return (
    <main className="workbench">
      <header className="topbar">
        <div>
          <p className="eyebrow">Task 3 · File-driven UI shell</p>
      ...
      <section className="toolbar">
        <input placeholder="Search SKU, title, brand…" value={query} onChange={(e) => { setQuery(e.target.value); setIndex(0) }} />
        ...
        <button className="ghost" onClick={() => setShowQa(!showQa)}><SlidersHorizontal size={16}/> Data QA</button>
...
function ProductImage({ item, image, review, saveReview, position }: any) {
  ...
  return <aside className="image-panel">
    ...
    <h2>{item.product.title}</h2><p>{item.product.brand}</p><code>{item.product.product_id}</code>
  </aside>
}
...
function VibePanel({ item, review, saveReview }: { item: ValidationItem; review: ProductReview; saveReview: (r: ProductReview) => void }) {
  ...
  return <section className="card"><h3>Suggested vibe validation</h3><p className="hint">Computed vector vibes and GPT suggestions are separate. Disagree using canonical vibe dropdowns only.</p><div className="vibe-list">{[...computed.slice(0,3), ...gpt].map((v, idx) => <VibeRow key={`${v.source}-${v.label}-${idx}`} vibe={v} review={review} saveReview={saveReview} />)}</div><details><summary>Show all 12 computed vibe scores</summary>{computed.map((v) => <div key={v.label} className="score-row"><span>{v.label}</span><progress max={100} value={v.score}/><b>{v.score.toFixed(1)}</b></div>)}</details></section>
}
