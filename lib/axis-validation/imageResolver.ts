...
export function resolveImage(product: Product): ImageResolution {
  ...
  if (image.startsWith('/source-truth/drive-image/')) {
    const fileId = image.split('/').pop()?.replace(/[^a-zA-Z0-9_-]/g, '') ?? ''
    if (fileId) {
      const src = `/product-image/drive/${encodeURIComponent(fileId)}`
      return { status: 'ok', src, candidates: [{ src, label: 'Source Truth Drive image', confidence: 'exact' }] }
    }
  }
