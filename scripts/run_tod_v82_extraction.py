...
DEFAULT_OUT = ROOT / "public/data/tod/tod_extractions_v8_2.json"
DEFAULT_PRODUCTS_OUT = ROOT / "public/data/tod/tod_products_extracted_249.json"
DEFAULT_REPORT = ROOT / "reports/TOD_V82_EXTRACTION_AUDIT_MAY25.md"
...
def ensure_defaults(row: dict[str, Any], ex: dict[str, Any], model: str, usage: Any = None) -> dict[str, Any]:
    ...
    ex.setdefault("schema_version", "8.2")
...
def main() -> int:
    ...
    load_env()
    client = OpenAI()
    guard = (ROOT / "config/GTF_Extraction_Prompt_v8.2_guard.md").read_text(encoding="utf-8")
    schema = load_json(ROOT / "config/GTF_Extraction_Schema_v8.2.json")
    ...
    report = [
        "# TOD v8.2/v8.3 GPT Extraction Audit — May 25, 2026",
        "",
        ...
        "",
        "## Gate",
        "- AR handoff is allowed only after this file reaches 249 outputs and validation issues are reviewed.",
