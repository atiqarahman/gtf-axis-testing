#!/usr/bin/env python3
"""Run AFROPOP v8.3 spot-check proof from live Source Truth handoff.

Outputs CTO proof artifacts under ~/.openclaw/workspace/gtf-review/v83-implementation/.
"""
...
OUT = WORKSPACE / "gtf-review" / "v83-implementation"
...
def build_prompt(row: dict, guard: str, schema: dict) -> str:
    ...
    return (
        guard
        + "\n\nReturn JSON following this v8.3 schema intent. Include legacy-compatible hard_attributes plus v8.3 fields.\n"
...
def extraction_key(ex: dict[str, Any]) -> str:
    return str(ex.get("brand_sku") or ex.get("gtf_sku") or ex.get("product_id") or "")


def canonical_meta(row: dict[str, Any], model: str, guard: str, schema: dict[str, Any], command: str, usage: Any = None) -> dict[str, Any]:
    return {
        "model": model,
        "prompt_guard_version": "v8.3-refinement-spec-may29",
        "prompt_guard_sha256": sha256_text(guard),
        "schema_version": "8.3",
...
def main() -> int:
    ...
    ap.add_argument("--all", action="store_true", help="Run all approved products in the Source Truth handoff instead of the 12-row spot-check subset")
    ap.add_argument("--prefix", default=None, help="Output filename prefix under v83-implementation")
    ap.add_argument("--force", action="store_true", help="Ignore final/partial checkpoints and re-extract all targeted rows")
    args = ap.parse_args()

    _load_key()
    client = OpenAI()
    guard = (ROOT / "config/GTF_Extraction_Prompt_v8.3_guard.md").read_text()
    schema = json.loads((ROOT / "config/GTF_Extraction_Schema_v8.3.json").read_text())
    ...
    rows = list(rows_by_sku.values()) if args.all else [rows_by_sku[s] for s in SELECTED if s in rows_by_sku]
    prefix = args.prefix or ("afropop_v83_full" if args.all else "afropop_v83_spotcheck")
    manifest = []
    for r in rows:
        manifest.append({
    ...
    extraction_path = OUT / f"{prefix}_extractions.json"
    partial_path = OUT / f"{prefix}_extractions.partial.json"
    raw_path = OUT / f"{prefix}_extractions.raw.json"
    failures_path = OUT / f"{prefix}_failures.json"
    command = " ".join(sys.argv)
    force = args.force or os.getenv("FORCE_GPT") == "1"

    extractions = []
    failures = []
    if not force and extraction_path.exists():
        extractions = json.loads(extraction_path.read_text(encoding="utf-8"))
        print(f"Reusing existing final {len(extractions)} extractions from {extraction_path}")
    elif not force and partial_path.exists():
        extractions = json.loads(partial_path.read_text(encoding="utf-8"))
        print(f"Resuming from partial {len(extractions)} extractions at {partial_path}")
    completed = {extraction_key(ex) for ex in extractions}
    if failures_path.exists() and not force:
        failures = json.loads(failures_path.read_text(encoding="utf-8"))

    for idx, row in enumerate(approved, 1):
        sku = row.get("brand_sku") or row.get("gtf_sku")
        if sku in completed:
            print(f"[{idx}/{len(approved)}] reuse {sku}", flush=True)
            continue
        if not row.get("brand_category"):
            ex = {"brand_sku": sku, "extraction_status": "blocked_missing_brand_category"}
            extractions.append(ex); completed.add(sku)
            atomic_json_write(partial_path, extractions)
            continue
        print(f"[{idx}/{len(approved)}] extracting {sku} {row.get('brand_category')}...", flush=True)
        try:
            imgs = []
            ...
            ex.setdefault("extraction_version", "v8.3")
            ex.setdefault("schema_version", "8.3")
            ex.setdefault("meta", {})
            ex["meta"].update(canonical_meta(row, "gpt-4o", guard, schema, command, resp.usage))
            ...
            ex["v83_raw_response"] = raw
            extractions = [old for old in extractions if extraction_key(old) != sku]
            extractions.append(ex)
            completed.add(sku)
            atomic_json_write(partial_path, extractions)
            atomic_json_write(raw_path, extractions)
        except Exception as exc:
            failure = {"brand_sku": sku, "gtf_sku": row.get("gtf_sku"), "product_name": row.get("product_name"), "error": repr(exc), "at": datetime.now(timezone.utc).isoformat()}
    ...
    atomic_json_write(OUT / f"{prefix}_extractions.json", extractions)
    ...
    with (OUT / f"{prefix}_search_tests.csv").open("w", newline="", encoding="utf-8") as f:
    ...
    (OUT / f"{prefix}_variant_c_payloads.json").write_text(json.dumps(variant_payloads, indent=2), encoding="utf-8")

    # Verdicts
    if args.all:
        tc01_pass = all(r["review_status"] == "approved" and r["brand_category"] and r["final_primary_image"] and r["final_secondary_image"] for r in manifest if r["brand_sku"] != NEGATIVE_SKU)
    else:
        tc01_pass = len(rows) == 12 and sum(1 for r in manifest if r["review_status"] == "approved" and r["brand_sku"] != NEGATIVE_SKU and r["brand_category"] and r["final_primary_image"] and r["final_secondary_image"]) == len(approved) and rows_by_sku.get(NEGATIVE_SKU, {}).get("review_status") == "sent_to_review"
    ...
    variant_pass = len(variant_payloads) == len(approved) and all(p["brand_category"] and p["final_primary_image"] and p["final_secondary_image"] for p in variant_payloads)
    overall = tc01_pass and tc02_pass and tc03_pass and tc05_pass and search_pass and variant_pass

    report = []
    report.append(("# AFROPOP v8.3 Full Extraction Report\n" if args.all else "# AFROPOP v8.3 Spot-Check Proof Report\n"))
    report.append(f"Date: 2026-05-29\nRun ID: `{RUN_ID}`\nExtraction model: `gpt-4o`\nPrompt: `GTF_Extraction_Prompt_v8.3_guard.md`\n")
    report.append("## Executive verdict\n")
    report.append(f"- Status: {'PASS' if overall else 'FAIL / PARTIAL'}\n")
    report.append(f"- Full extraction mode: {'YES' if args.all else 'NO'}\n")
    report.append(f"- Can proceed to next validation: {'YES' if overall else 'REVIEW GATES'}\n")
    ...
    (OUT / f"{prefix.upper()}_REPORT.md").write_text("\n".join(report), encoding="utf-8")
    print("OVERALL", "PASS" if overall else "FAIL/PARTIAL")
    print("Report", OUT / f"{prefix.upper()}_REPORT.md")
